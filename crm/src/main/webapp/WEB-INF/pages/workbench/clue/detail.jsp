...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#searchActivityTxt").keyup(function () {
			...
			// 发送请求
			$.ajax({
				url: 'workbench/clue/selectActivityForDetailByNameExcludeClueId.do',
				data: {
		...
		$("#saveBundActivityBtn").click(function () {
			...
			ids += "clueId=${clue.id}"; // activityIds=xxx&activityIds=xxx&clueId=xxx
			// 发送请求
			$.ajax({
				url: 'workbench/clue/insertBundRelation.do',
				data:ids,
				type:'post',
				dataType: 'json',
				success:function (data) {
		...
		$("#relationTBody").on("click", "a", function () {
			...
			if (window.confirm("确定删除吗?")) {
				$.ajax({
					url: 'workbench/clue/unBundRelation.do',
					data: {
			...
			$.ajax({
				...
				success: function (data) {
					if (data.code == '0') {
						...
						htmlStr += "        <font color=\"gray\">线索</font> <font color=\"gray\">-</font> <b>${clue.fullname}${clue.appellation}-${clue.company}</b> <small style=\"color: gray;\"> "+data.data.createTime+" 由${sessionScope.sessionUser.name}创建</small>";
